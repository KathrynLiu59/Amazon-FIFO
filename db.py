...
def df_insert(df: pd.DataFrame, table: str, truncate_first=False):
    ...
    values = [tuple(row[c] if not pd.isna(row[c]) else None for c in cols) for _, row in df.iterrows()]
    with get_conn() as conn, conn.cursor() as cur:
        if truncate_first:
...
def df_upsert(df: pd.DataFrame, table: str, conflict_cols: list[str]):
    ...
    with get_conn() as conn, conn.cursor() as cur:
        ...
        rows = [tuple(row[c] if not pd.isna(row[c]) else None for c in cols) for _, row in df.iterrows()]
