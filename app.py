# app.py — Amazon FIFO Cost Portal (full v3)
...
with tabs[2]:
    ...
    st.caption("We read: date/time, settlement type, order id, marketplace, sku, quantity")
    ...
    def normalize_csv(df: pd.DataFrame):
        ...
        return out[(out["raw_time"] >= s) & (out["raw_time"] < e)]

    if file and st.button("Import & Compute", type="primary"):
        df0 = pd.read_csv(file)
        data = normalize_csv(df0)
        if data.empty:
...
with tabs[3]:
    ...
    if c2.button("Show month history"):
        hist = run_sql("select rev_id, snapshot_at, reason, fob_sum, freight_sum, clearance_sum, duty_sum, orders from month_history where ym=%s order by rev_id desc", (ym_q,))
        if hist:
